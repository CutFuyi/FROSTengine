    ...
    /*
        ...
        Okna można skalować z całą ich zawartością, od ich wersji domyślnej, w tym pasek minimalizacji i związane z tym animacje chowania i wychodzenia.
        Dać zmienną która zatrzymuje co się dzieje w oknie jeżeli okno nie będzie pierwsze na liście, to samo z zatrzymywaniem dźwięków itd.
        Zrobić okna przypisane które nakładają się na okno ojca i blokują co jest za nimi dopóki nie znikną, przemieszczają się z oknem itd,
        Silnik 3D nie liczy wszystkich linii i jest system który tworzy w prosty sposób linie między
        Ustawienia rozdzielczości dla silnika 3D które na sucho zmieniają działanie systemu rysowania i promieni
        Hitbox specjalny dla budynków żeby można była używać terenu pod kątem i budynków np 3 osobne hitboxy (2 kółka i kwadrat)
        Sliders
        System fizyki dla gry 2D platformowej.
        Bardziej optymalny system wczytywania danych.
        Rozwiązać problem pierwszego odpalenia dźwięków.
        Zmniejszenie jakości promienia z odległością (optymalizacja)
        Zmniejszenie rysowanych promieni dla dalszych obiektów (Test)
        Widok do góry i dołu, sterowanie FPS pełne
        Podłoga Teksturowana
        Sufit Teksturowany
        Zaciemnianie na podstawie odległości, zaciemnienie horyzontu.
        Dodać Okrągłe HitBoxy Przycisków
        Dodać Okrągłe Obiekty dla systemu 3D
        Dodać Okrągłe Obiekty dla kolizji Open World
        Dodać wypełnienia obiektów 3D
        Dodać obiekty płaskie
        Dodać sufit z różnymi wysokościami / Skybox
        Dachy wystające jednokolorowe
        Usuwana niepotrzebne linie za obiektami (tworzy dodatkowe linie jeżeli jest przerwa).
        Okno ustawień zaawansowanych gdzie jest ciągle wyświetlające się demo w pętli, gracz może ustawić konkretne liczby systemu 3D, są limity i maks. (info o domyślnej wartości zawsze obok).
        Tryb Edycji dla tworzenia planszy pod system 3D, mamy okno systemu open world gdzie możemy wkładać obiekty w siatce, później można zapisac mapę jako 
        plik który wczytuje system. (bardzo mała skala pod optymalizacje, w praktyce system może działać w obu trybach na raz - widok 3D i Open World).
        System pathfinding
        Obiekty 2D na różnych wysokościach w środowisku 3D które mogą zmieniać swój wygląd zależnie od naszego kierunku patrzenia
        Efekty pogodowe (mgła, noc, śnieg, deszcz) - które mogą wpływać procentowo na widoczność i dodawać efekt na renderowane obiekty
        Zmienna - myszka znika będąc na oknie renderu 3D
        System Cutscenek Open World z Render 3D
        Wypełnienie obiektów liniami, później tekstury
